...
int main() {
    ...
    if (planilha != NULL) {
        ...
        /* ======= BLOCO DAS BUSCAS (DFS / BFS) + MEDIÇÃO DE TEMPOS ======= */
        {
            char texto_busca[10];
            int col_b;
            int lin_b;
            ...
            if (fgets(linha_entrada, sizeof(linha_entrada), stdin) != NULL &&
                ...
                if (converter_texto_para_posicao(texto_busca, &col_b, &lin_b) == 0) {
                    ...
                    /* Uma execução normal só para mostrar o percurso */
                    busca_profundidade_dependencias(planilha, indice_busca);
                    busca_largura_dependencias(planilha, indice_busca);

                    /* ===== Medição de tempo (30 repetições) ===== */
                    {
                        int repeticao;
                        clock_t inicio, fim;
                        double soma_insercao = 0.0;
                        double soma_bfs = 0.0;
                        double soma_dfs = 0.0;

                        /* Usamos uma celula de teste para insercao (H20 = última) */
                        int indice_teste_insercao = TOTAL_CELULAS - 1; /* H20 */
                        int indice_dep_exemplo = 0;                    /* A1 */

                        for (repeticao = 0; repeticao < 30; repeticao++) {
                            /* (a) Inserção de dados no grafo - simulada em H20 -> A1 */
                            inicio = clock();
                            ...
                            /* (b) Busca em largura (BFS) a partir da célula escolhida */
                            inicio = clock();
                            busca_largura_dependencias(planilha, indice_busca);
                            fim = clock();
                            soma_bfs = soma_bfs +
                            ...
                            /* (c) Busca em profundidade (DFS) a partir da mesma célula */
                            inicio = clock();
                            busca_profundidade_dependencias(planilha, indice_busca);
                            fim = clock();
                            soma_dfs = soma_dfs +
                        ...
                        printf("\n=== Tempos medios apos 30 repeticoes ===\n");
